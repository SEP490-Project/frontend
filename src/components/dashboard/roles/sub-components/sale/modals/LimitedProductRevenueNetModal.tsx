...
import { getLimitedNetRevenueDetailsThunk } from "@/libs/stores/analyticDetailManager/thunk";
import type {
...
interface LimitedProductRevenueNetModalProps {
  isOpen: boolean;
  onClose: () => void;
  startDate?: string;
  endDate?: string;
...
export const LimitedProductRevenueNetModal: React.FC<LimitedProductRevenueNetModalProps> = ({
  isOpen,
  onClose,
  startDate,
  endDate,
  ...
  const { limitedNetRevenueDetails, loading } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getLimitedNetRevenueDetailsThunk(params));
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          ) : limitedNetRevenueDetails?.data && limitedNetRevenueDetails.data.length > 0 ? (
            <>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {limitedNetRevenueDetails.data.map((item) => (
                      <tr
                        key={item.id}
              ...
              {/* Pagination */}
              {limitedNetRevenueDetails.pagination && (
                <div className="flex justify-between items-center mt-4">
                  <div className="text-sm text-gray-600">
                    Total: {limitedNetRevenueDetails.pagination.total} items | Revenue:{" "}
                    <span className="font-bold">
                      {convertNumberToCurrency(String(limitedNetRevenueDetails.total_revenue))}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      disabled={!limitedNetRevenueDetails.pagination.has_prev}
                      onClick={() => setPage(page - 1)}
                    >
                      Previous
                    </Button>
                    <span className="px-4 py-2 text-sm">
                      Page {page} of {limitedNetRevenueDetails.pagination.total_pages}
                    </span>
                    <Button
                      variant="outline"
                      disabled={!limitedNetRevenueDetails.pagination.has_next}
                      onClick={() => setPage(page + 1)}
                    >
                      Next
                    </Button>
...
export default LimitedProductRevenueNetModal;
