...
import { getStandardProductRevenueDetailsThunk } from "@/libs/stores/analyticDetailManager/thunk";
import type {
...
interface StandardProductRevenueProps {
  isOpen: boolean;
  onClose: () => void;
  startDate?: string;
  endDate?: string;
...
export const StandardProductRevenue: React.FC<StandardProductRevenueProps> = ({
  isOpen,
  onClose,
  startDate,
  endDate,
  ...
  const { standardProductRevenueDetails, loading } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getStandardProductRevenueDetailsThunk(params));
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          ) : standardProductRevenueDetails?.data &&
            standardProductRevenueDetails.data.length > 0 ? (
            <>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {standardProductRevenueDetails.data.map((item) => (
                      <tr
                        key={item.id}
              ...
              {/* Pagination */}
              {standardProductRevenueDetails.pagination && (
                <div className="flex justify-between items-center mt-4">
                  <div className="text-sm text-gray-600">
                    Total: {standardProductRevenueDetails.pagination.total} items | Revenue:{" "}
                    <span className="font-bold">
                      {convertNumberToCurrency(String(standardProductRevenueDetails.total_revenue))}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      disabled={!standardProductRevenueDetails.pagination.has_prev}
                      onClick={() => setPage(page - 1)}
                    >
                      Previous
                    </Button>
                    <span className="px-4 py-2 text-sm">
                      Page {page} of {standardProductRevenueDetails.pagination.total_pages}
                    </span>
                    <Button
                      variant="outline"
                      disabled={!standardProductRevenueDetails.pagination.has_next}
                      onClick={() => setPage(page + 1)}
                    >
                      Next
                    </Button>
...
export default StandardProductRevenue;
