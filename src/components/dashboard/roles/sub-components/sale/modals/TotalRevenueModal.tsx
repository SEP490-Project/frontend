...
import { getTotalRevenueDetailsThunk } from "@/libs/stores/analyticDetailManager/thunk";
import type {
...
export const TotalRevenueModal: React.FC<TotalRevenueModalProps> = ({
  isOpen,
  onClose,
  startDate,
  endDate,
  ...
  const { totalRevenueDetails, loading } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getTotalRevenueDetailsThunk(params));
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          ) : totalRevenueDetails?.data && totalRevenueDetails.data.length > 0 ? (
            <>
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    {totalRevenueDetails.data.map((item) => (
                      <tr
                        key={item.id}
              ...
              {/* Pagination */}
              {totalRevenueDetails.pagination && (
                <div className="flex justify-between items-center mt-4">
                  <div className="text-sm text-gray-600">
                    Total: {totalRevenueDetails.pagination.total} items | Revenue:{" "}
                    <span className="font-bold">
                      {convertNumberToCurrency(
                        String(totalRevenueDetails.total_revenue.toFixed(2)),
                      )}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      variant="outline"
                      disabled={!totalRevenueDetails.pagination.has_prev}
                      onClick={() => setPage(page - 1)}
                    >
                      Previous
                    </Button>
                    <span className="px-4 py-2 text-sm">
                      Page {page} of {totalRevenueDetails.pagination.total_pages}
                    </span>
                    <Button
                      variant="outline"
                      disabled={!totalRevenueDetails.pagination.has_next}
                      onClick={() => setPage(page + 1)}
                    >
                      Next
                    </Button>
