...
interface ConfirmPreOrderProps {
  preOrder: PreOrderData;
  onConfirm: () => void;
  onCancel: (reason: string, file: File) => void;
}

const ConfirmPreOrder = ({ preOrder, onConfirm, onCancel }: ConfirmPreOrderProps) => {
  ...
  const handleCancelPreOrder = async () => {
    ...
    setIsSubmitting(true);
    try {
      await onCancel(reason.trim(), selectedFile);
      setAction(null);
      setReason("");
      handleRemoveFile();
    } finally {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Review and confirm or cancel this paid pre-order</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <p className="font-medium">
                {preOrder.is_self_picked_up ? "Self Pick-up" : "Shipping Delivery"}
              </p>
            </div>
          </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700 mt-1">
                      Please review the pre-order details and choose to confirm or cancel this
                      pre-order.
                    </p>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium text-green-900">Confirm Pre-Order</p>
                    <p className="text-sm text-green-700 mt-1">
                      The pre-order will be confirmed and proceed to the next stage.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-medium text-red-900">Cancel Pre-Order & Process Refund</p>
                    <p className="text-sm text-red-700 mt-1">
                      This pre-order has been paid. Please provide cancellation reason and upload
                      refund proof.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-medium">{preOrder.user_bank_name}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Account:</span>
                      <span className="ml-2 font-medium">{preOrder.user_bank_account}</span>
                    </div>
                    <div className="col-span-2">
                      <span className="text-gray-500">Account Holder:</span>
                      <span className="ml-2 font-medium">{preOrder.user_bank_account_holder}</span>
                    </div>
                  </div>
                </div>
              )}
